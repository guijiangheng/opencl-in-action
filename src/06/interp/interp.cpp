...
cl_program buildProgram(cl_context context,
	...
	char options[20];
	sprintf(options, "-DSCALE=%u", SCALE_FACTOR);
...
void read_image(const char* filename,
				png_bytep* input, png_bytep* output,
				size_t* w, size_t* h) {
	FILE* file = fopen(filename, "rb");
...
int main() {
	...
	auto program = buildProgram(context, device, "interp.cl");
	auto kernel = clCreateKernel(program, "interp", NULL);

	png_bytep input, output;
	size_t width, height;
	read_image("input.png", &input, &output, &width, &height);
	...
	auto src_img = clCreateImage2D(
				   ...
				   &format, width, height, 0, input, NULL);
	auto dst_img = clCreateImage2D(
				   context, CL_MEM_READ_ONLY,
				   &format, width * SCALE_FACTOR, height * SCALE_FACTOR,
				   0, NULL, NULL);
	...
	size_t origin[] = { 0, 0, 0 };
	size_t region[] = { width * SCALE_FACTOR, height * SCALE_FACTOR, 1 };
	clEnqueueReadImage(queue, dst_img, CL_TRUE, origin, region, 0, 0, output, 0, NULL, NULL);
	write_image("output.png", output, region[0], region[1]);
	free(input);
	free(output);
